 ...
 * performs the sieveAlgorithm on its own chunk using the seed primes
 */
void* threadSieve(void* arg) {
    ThreadData* data = (ThreadData*)arg;

    std::vector<int> seedCopy = *(data->seed);

    for (int prime : seedCopy) {
        sieveAlgorithm(&(data->chunk), prime);
    }
...
int main(int argc, char *argv[]) {
    ...
    for (int i = 0; i < threads; ++i) {
        threadData[i].chunk = chunkCollection[i];
        threadData[i].seed = &seed;
        pthread_create(&threadIds[i], nullptr, threadSieve, &threadData[i]);
    }

    // Step 4: Master waits for threads to finish and collects unmarked numbers.
    for (int i = 0; i < threads; ++i) {
   ...
   for (int i = 0; i < threads; ++i) {
        seed.insert(seed.end(), threadData[i].chunk.begin(), threadData[i].chunk.end());
    }
